...
- **类型生成**：`pnpm generate:types`（依次调用 Cloudflare 与 Payload 类型生成脚本）
- **代码检查**：`pnpm lint:fix`（优先使用），或 `pnpm lint`
- **Payload CMS**：`pnpm payload`（本地 CMS 操作/迁移）
- **清理**：`pnpm clean`（移除 `.next`、`.open-next`、`data/app`）

## 环境与配置

- Node.js ≥ 22、pnpm ≥ 10，并确保 `pnpm` 在 PATH 中。
- 首次生成类型：`pnpm generate:types`。

## 本地运行要点

- **Next.js**：`pnpm dev` 并访问 `http://localhost:3000`（前台）与 `/admin`（Payload UI）。
- **Claude Agent**：`pnpm dev:agent`，HTTP 入口 `POST http://localhost:2442/chat`，WebSocket 入口 `ws://localhost:2442/ws`，请求体与 SDK 参数保持一致。
- **Cloudflare Worker**：`pnpm dev:worker`，依赖本地 Docker 提供持久化，HTTP 入口 `http://localhost:8787/chat`，WebSocket 入口 `ws://localhost:8787/ws`。
